...
export async function getServerSideProps(context) {
    ...
    if (!profile || profile?.rol !== 'koordinator') return { redirect: { destination: '/', permanent: false } };
    ...
    if (!reportData) return { props: { report: null, pdfUrl: null } };

    const { data: assignmentCheck } = await supabaseAdmin.from('koordinator_sorumluluklari').select('id').eq('koordinator_id', user.id).eq('sorumlu_id', reportData.sorumlu_id).single();
    if (!assignmentCheck) return { props: { report: null, pdfUrl: null } };

    const { data: signedUrlData } = await supabaseAdmin.storage.from('raporlar').createSignedUrl(reportData.pdf_storage_path, 3600);
