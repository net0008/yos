...
export default async function handler(req, res) {
    // En başta header'ı set et — ne olursa olsun JSON dön
    res.setHeader('Content-Type', 'application/json');

    // Tüm handler'ı tek try/catch içine al
    // Herhangi bir unhandled exception HTML değil JSON döndürsün
    try {
        if (req.method !== 'POST') {
        ...
        // İlçedeki sorumlular — ilike ile case-insensitive
        ...
        if (!sorumlular || sorumlular.length === 0) {
            return res.status(200).json({
                success: true,
                message: `"${ilceAdi}" ilçesinde henüz kayıtlı sorumlu yok. Sorumlu yüklendiğinde atama eşleşecek.`,
                atanan: 0,
            });
        }
        ...
        // Son çare: beklenmedik her hata için JSON dön
